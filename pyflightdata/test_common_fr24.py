...
class TestCommonFR24(object):
    ...
    def test_get_raw_aircraft_image_data(self):
        assert get_raw_aircraft_image_data(REG_BASE.format('VT-ALL','',FlightData.AUTH_TOKEN,1,100)).__len__() >= 0

    def test_process_raw_aircraft_image_data(self):
        data = get_raw_aircraft_image_data(REG_BASE.format('VT-ALL','',FlightData.AUTH_TOKEN,1,100))
        assert data.__len__() >= 0
        result = process_raw_aircraft_image_data(data)
        assert result.__len__() >= 0

    def test_get_raw_aircraft_image_data_dummy(self):
        assert get_raw_aircraft_image_data(REG_BASE.format('VT-A','',FlightData.AUTH_TOKEN,1,100)).__len__() == 0

    def test_process_raw_aircraft_image_data_dummy(self):
        data = get_raw_aircraft_image_data(REG_BASE.format('VT-A','',FlightData.AUTH_TOKEN,1,100))
        assert data.__len__() == 0
        result = process_raw_aircraft_image_data(data)
        assert result.__len__() == 0

    def test_get_raw_aircraft_info_data(self):
        assert get_raw_aircraft_info_data(REG_BASE.format('VT-ALL','',FlightData.AUTH_TOKEN,1,100)).__len__() >= 0

    def test_process_raw_aircraft_info_data(self):
        data = get_raw_aircraft_info_data(REG_BASE.format('VT-ALL','',FlightData.AUTH_TOKEN,1,100))
        assert data.__len__() >= 0
        result = process_raw_aircraft_info_data(data)
        assert result.__len__() >= 0

    def test_get_raw_aircraft_info_data_dummy(self):
        assert get_raw_aircraft_info_data(REG_BASE.format('VT-A','',FlightData.AUTH_TOKEN,1,100)).__len__() == 0

    def test_process_raw_aircraft_info_data_dummy(self):
        data = get_raw_aircraft_info_data(REG_BASE.format('VT-A','',FlightData.AUTH_TOKEN,1,100))
        assert data.__len__() == 0
        result = process_raw_aircraft_info_data(data)
        assert result.__len__() == 0

    def test_get_aircraft_data(self):
        assert get_aircraft_data(REG_BASE.format('VT-ALL','',FlightData.AUTH_TOKEN,1,100)).__len__() >= 0
    ...
    def test_process_raw_airline_fleet_data(self):
        data = get_raw_airline_fleet_data(REG_BASE.format('lufthansa-dlh','',FlightData.AUTH_TOKEN,1,100))
        assert data.__len__() >= 0
    ...
    def test_process_raw_airline_flight_data(self):
        data = get_raw_airline_flight_data(FLT_BASE.format('air-india-aic','',FlightData.AUTH_TOKEN,1,100))
        assert data.__len__() >= 0
